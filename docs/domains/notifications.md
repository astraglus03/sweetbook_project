...
- 주문 상태 변경: 주문자에게 발송 (Sweetbook 웹훅 수신 시)

## Sweetbook 웹훅 → 알림 매핑

> 웹훅 기술 상세(서명 검증, 재시도 정책, 엔드포인트 등)는 `.claude/rules/backend/external-api.md` 참조. 여기는 **어떤 이벤트를 어떤 알림으로 변환할지**만 정의.

| Sweetbook 이벤트 | 알림 type | 사용자 표시 문구 | payload 활용 필드 |
|-----------------|-----------|----------------|------------------|
| `order.created` | `ORDER_STATUS` | "주문이 접수됐어요" | `orderUid` |
| `order.cancelled` | `ORDER_STATUS` | "주문이 취소됐어요. {refundAmount}원 환불됩니다" | `refundAmount`, `cancelReason` |
| `order.restored` | `ORDER_STATUS` | "주문이 복구됐어요" | `orderUid` |
| `production.confirmed` | `ORDER_STATUS` | "제작이 확정됐어요. 예상 인쇄일 {date}" | **`expectedPrintDate`** |
| `production.started` | (skip) | — | 빈번한 이벤트라 알림 생략 |
| `production.completed` | `ORDER_STATUS` | "제작이 완료됐어요. 곧 배송됩니다" | `orderUid` |
| `shipping.departed` | `ORDER_STATUS` | "발송됐어요. 운송장 {trackingNumber} ({carrier})" | **`trackingNumber`**, **`carrierCode`** |
| `shipping.delivered` | `ORDER_STATUS` | "배송이 완료됐어요 🎁" | `deliveredAt` |

**🔴 isTest 이벤트는 알림 스킵**: Sandbox에서 `POST /webhooks/test`로 발생한 이벤트는 payload에 `isTest: true`가 포함됨 → 실사용자에게 알림 발송하지 않음.
